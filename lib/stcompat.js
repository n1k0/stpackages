...
function parseCompatData(data) {
  ...
  var labels = {
    'Needs Extra Steps': 'extra_steps',
    'Working': 'working',
    'Not working': 'errors',
    'Works With Errors': 'errors'
  };
  data.split('\n\n## ').slice(1, 5).forEach(function(rawSection) {
    var lines = rawSection.split('\n');
    var sectionName = lines[0].split(' ##')[0].trim();
    var compat = sectionName in labels ? labels[sectionName] : defaultLabel;
